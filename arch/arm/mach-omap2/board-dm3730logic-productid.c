/*
 * linux/arch/arm/mach-omap2/board-dm3730logic-product-id.c
 *
...
static int valid_product_id_has_wifi_config_data;  // !0 if has TI WiLink
...
static inline int dm3730logic_is_product_data_valid(void)
{
	return product_id_data_valid;
}

static int dm3730logic_extract_header_version(struct product_id_data *p, int *header_version)
{
...
int dm3730logic_extract_product_id_part_number(struct product_id_data *p, char *buf, int buflen)
{
	int size;
...
static int extract_part_number(void)
{
	...
	ret = dm3730logic_extract_product_id_part_number(&product_id_data, buf, sizeof(buf));
	if (ret)
		return -EINVAL;
...
int dm3730logic_get_product_id_part_number(u32 *part_number)
{
...
int dm3730logic_extract_serial_number(struct product_id_data *p, char *buf, int buflen)
{
	buf[0] = '\0';
...
int dm3730logic_extract_model_number_revision(struct product_id_data *p, char *buf, int buflen)
{
	int i;
...
int dm3730logic_extract_lan_ethaddr(u8 *ethaddr)
{
	int ret;
	ret = extract_product_id_mac_address(&product_id_data, 0, ethaddr);
	return !ret;
}
EXPORT_SYMBOL(dm3730logic_extract_lan_ethaddr);

/* Extract the WiFi ethaddr, and return !0 if its valid */
int dm3730logic_extract_wifi_ethaddr(u8 *ethaddr)
{
	int ret;
	ret = extract_product_id_mac_address(&product_id_data, 1, ethaddr);
	return !ret;
}
EXPORT_SYMBOL(dm3730logic_extract_wifi_ethaddr);
...
void valid_data_extract_dump(struct product_id_data *p)
{
	...
	ret = dm3730logic_extract_product_id_part_number(p, buf, sizeof(buf));
	if (!ret)
		printk(KERN_INFO "Part Number  : %s\n", buf);

	ret = dm3730logic_extract_model_number_revision(p, buf, sizeof(buf));
	if (!ret)
		printk(KERN_INFO "Model Name   : %s\n", buf);

	ret = dm3730logic_extract_serial_number(p, buf, sizeof(buf));
	if (!ret)
	...
	/* If SOM has TI WiLink, then it has WiFi config data as well */
	if (dm3730logic_has_wilink_wifi_module()) {
		valid_product_id_has_wifi_config_data = 1;
		printk(KERN_INFO "WiLink Module found\n");
	}
}

/* Check the product data passed in from u-boot at the start of SRAM. */
void dm3730logic_fetch_sram_product_id_data(void)
{
	...
	if (dm3730logic_extract_header_version(&product_id_data, &header_version)) {
	...
	/* No need to fix endianess as its been done in u-boot */
...
ssize_t product_id_show_part_number(struct class *class, char *buf)
{
	...
	dm3730logic_extract_product_id_part_number(&product_id_data, buf, 128);
	len = strlen(buf);
	buf[len++] = '\n';
	return len;
}
...
ssize_t product_id_show_model_name(struct class *class, char *buf)
{
	...
	dm3730logic_extract_model_number_revision(&product_id_data, buf, 128);
	len = strlen(buf);
	buf[len++] = '\n';
	return len;
}
...
ssize_t product_id_show_serial_number(struct class *class, char *buf)
{
	...
	dm3730logic_extract_serial_number(&product_id_data, buf, 128);
	len = strlen(buf);
	buf[len++] = '\n';
	return len;
}
...
/* Is this a -11 LV SOM? */
static int dm3730logic_is_dash_11_module(void)
{
...
// On Torpedo 1017155, audio mute is on GPIO_17
#define TWL4030_TORPEDO_EXTERNAL_AUDIO_MUTE_GPIO	17
...
int dm3730logic_external_mute_gpio(void)
{
	/* If we're a torpedo, we use GPIO17 for external mute */
	if (machine_is_dm3730_torpedo())
		return TWL4030_TORPEDO_EXTERNAL_AUDIO_MUTE_GPIO;
	else
...
int dm3730logic_has_wilink_wifi_module(void)
{
	...
	if (!wilink_probed) {
		...
		if (gpio_direction_input(OMAP_DM3730LOGIC_WIFI_PMENA_GPIO) < 0)
			printk("%s:%d\n", __FUNCTION__, __LINE__);
...
/*
 * Return !0 if SOM has a isp176x on it, or is used with one.  Currently we
 * assume all -11 variants do *not* have the isp1670 and everything else
 * does, have to fix in the future */
int dm3730logic_has_isp176x(void)
{
	/* No valid data, then no isp176x */
	if (!dm3730logic_is_product_data_valid())
		return 0;
	...
	if (dm3730logic_is_dash_11_module())
		return 0;

	/* Its not a -11 SOM, so we must have the isp176x */
	return 1;

}

static int __init dm3730logic_fetch_production_data(void)
{
	int valid;
	/* TARR - Cloud surfer stuff init */
	//extern void dm3730logic_init_productid_specifics(void);
	extern void cloudsurfer_init_productid_specifics(void);
	...
	cloudsurfer_init_productid_specifics();

	return valid;
}
subsys_initcall_sync(dm3730logic_fetch_production_data);
