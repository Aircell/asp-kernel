...
struct omap_sdrc_params dm3730logic_sdrc_params[4];

/* Timings are for the -6 parts */
static const struct sdram_timings dm3730logic_timings[] = {
	{
		.casl = 3,
...
#define SDRC_SET_ONE(reg, st, end, field, rate) \
	if (set_sdrc_timing_regval((reg), (st), (end), \
			dm3730logic_timings->field, (rate), #field) < 0) \
		err = -1;
#else
#define SDRC_SET_ONE(reg, st, end, field, rate) \
	if (set_sdrc_timing_regval((reg), (st), (end), \
			dm3730logic_timings->field) < 0) \
		err = -1;
#endif
...
#define SDRC_SET_ONE_PS(reg, st, end, field, rate) \
	if (set_sdrc_timing_regval_ps((reg), (st), (end), \
			dm3730logic_timings->field, (rate)) < 0) \
		err = -1;
#endif
...
static int sdrc_timings(int id, long rate)
{
	...
	printk("%s: l3_rate %ld tRef %d\n", __FUNCTION__, l3_rate,
		dm3730logic_timings[0].tREF);
#endif
	rfr = dm3730logic_timings[0].tREF * ticks_per_ms / 1000000;
	if (rfr > 65535 + 50)
		rfr = 65535;
	else
		rfr -= 50;
	...
	dm3730logic_sdrc_params[id].rate = rate;
	dm3730logic_sdrc_params[id].actim_ctrla = actim_ctrla;
	dm3730logic_sdrc_params[id].actim_ctrlb = actim_ctrlb;
	dm3730logic_sdrc_params[id].rfr_ctrl = rfr_ctrl;
	dm3730logic_sdrc_params[id].mr = 0x32;

	dm3730logic_sdrc_params[id + 1].rate = 0;

	return err;
}

struct omap_sdrc_params *dm3730logic_get_sdram_timings(void)
{
	int err;
	...
	return &dm3730logic_sdrc_params[0];
}

void dump_dm3730logic_timings(void)
{
	int i;
	struct omap_sdrc_params *p;
	for (i=0; i<ARRAY_SIZE(dm3730logic_sdrc_params); i++) {
		p = &dm3730logic_sdrc_params[i];
		if (p->rate) {
