...
void valid_data_extract_dump(struct product_id_data *p)
{
	...
	/* If SOM has Murata, then it has WiFi config data as well */
...
int omap3logic_has_murata_wifi_module(void)
{
	...
	/* If earlier than rev3, then can't have a Murata */
	if (header_version < LOGIC_HEADER_VERSION_3) {
		printk("%s: %d < %d\n", __FUNCTION__, header_version, LOGIC_HEADER_VERSION_3);
		return 0;
	}
