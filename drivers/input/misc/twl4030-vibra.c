...
static void vibra_play_work(struct work_struct *work)
{
   ...
   pwm = info->speed;
#ifdef VDEBUG
	printk(KERN_INFO "entered vibra_play_work: dir = 0x%x, speed = %d\n", dir, pwm);
#endif
   twl_i2c_read_u8(TWL4030_MODULE_AUDIO_VOICE,
           &reg, TWL4030_REG_VIBRA_CTL);
   if (pwm && (!info->coexist || !(reg & TWL4030_VIBRA_SEL))) {
#ifdef VDEBUG
	printk(KERN_INFO "about to set direction and speed.\n");
#endif
       ...
       twl_i2c_read_u8(TWL4030_MODULE_AUDIO_VOICE,
               &reg, TWL4030_REG_VIBRA_CTL);
#ifdef VDEBUG
     printk(KERN_INFO "reg value = %x", reg);
#endif 
       reg = (dir) ? (reg | TWL4030_VIBRA_DIR) :
           (reg & ~TWL4030_VIBRA_DIR);
#ifdef VDEBUG
	int tmp = 0;
	twl_i2c_read_u8(TWL4030_MODULE_LED, &tmp, 0);
	printk(KERN_INFO "LEDEN = %x\n", tmp);
	tmp = 0;
	twl_i2c_write_u8(TWL4030_MODULE_LED, &tmp, 0);
	twl_i2c_read_u8(TWL4030_MODULE_LED, &tmp, 0);
	printk(KERN_INFO "LEDEN updated = %x\n", tmp);
...
#ifdef VDEBUG
	printk(KERN_INFO "entered vibra_play: speed = 0x%x, direction = 0x%x\n", effect->u.rumble.weak_magnitude, effect->direction);
#endif
...
#ifdef VDEBUG
	printk(KERN_INFO "about to enter work queue: speed = 0x%x, direction = 0x%x\n", info->speed, info->direction);
#endif
...
#ifdef VDEBUG
printk(KERN_INFO "successfully opened the vibrator device.\n");
#endif
   return 0;
}
