...
omap2_mcspi_txrx_pio(struct spi_device *spi, struct spi_transfer *xfer)
{
	...
	if (word_len <= 8) {
		...
		do {
			...
			if(spi->quirks & SPI_QUIRK_BRF6300) {
				if (!mcspi->first_write_issued && (count == c + 4))
					udelay(50);
			}
		} while (c);
	} else if (word_len <= 16) {
...
static void omap2_mcspi_work(struct work_struct *work)
{
	...
	while (!list_empty(&mcspi->msg_queue)) {
		...
		if (spi->quirks & SPI_QUIRK_BRF6300) {
			omap2_mcspi_set_master_mode(spi->master);  
		}
		...
		list_for_each_entry(t, &m->transfers, transfer_list) {
			...
			if (t->len) {
				...
				if ((!spi->quirks & SPI_QUIRK_BRF6300) && (m->is_dma_mapped ||
					t->len >= DMA_MIN_BYTES ||
						mcspi->dma_mode))
			...
			if(spi->quirks & SPI_QUIRK_BRF6300 && !mcspi->first_write_issued)
				mcspi->first_write_issued = 1; 
...
static int __init omap2_mcspi_reset(struct omap2_mcspi *mcspi)
{
	...
	if (1 /* !spi->quirks & SPI_QUIRK_BRF6300 */) {
		if (mcspi->mcspi_mode == OMAP2_MCSPI_MASTER)
