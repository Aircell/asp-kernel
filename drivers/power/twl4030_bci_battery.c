...
struct twl4030_bci_device_info {
	...
	int			charge_rsoc;
	int			charge_status;
	enum twl4030_bb_voltage bb_voltage;
	enum twl4030_bb_current bb_current;
	enum twl4030_bb_chen	bb_chen;
 ...
 * battery charging.
 */
static int twl4030backupbatt_voltage_setup(struct twl4030_bci_device_info *di)
{
	u8 val;

	switch(di->bb_voltage)
	{
		default:
		case BB_VOLTAGE_25:
			val = 0;
	...
	{	
		...
		case BB_CURRENT_500:
			val |= 2;
			break;
		case BB_CURRENT_1000:
			val |= 3;
			break;
	}

	if(di->bb_chen == BB_CHARGE_ENABLED)
	{
		val |= BBCHEN;
	}

	/* Setup the battery for the proper voltage/current */
	return twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, val, REG_BB_CFG);
}

/*
 * Settup the twl4030 BCI module to enable battery charging.
 */
static int twl4030batt_voltage_setup(struct twl4030_bci_device_info *di)
{
	u8 val;
	int ret = 0;

#if 0
	// Turn of auto-charging
	twl4030charger_ac_en(DISABLE);

	// Update charging current to 2x
	val = 0x20;
	ret = twl_i2c_write_u8(TWL4030_MODULE_MAIN_CHARGE, val,
			       REG_BCICTL1);
	if (ret) {
		printk("%s: Unable to write BCICTL\n", __FUNCTION__);
		return ret;
	}
	...
	val = 0x90;
	ret = twl_i2c_write_u8(TWL4030_MODULE_INTBR, val,
			       REG_GPBR1);
	if (ret)
		return ret;
...
static int __init twl4030_bci_battery_probe(struct platform_device *pdev)
{
	...
	di->bb_voltage = pdata->bb_voltage;
	...
	ret = twl4030backupbatt_voltage_setup(di);
	if (ret)
