...
static int isp1763_init_device(struct isp1763_dev *dev)
{
	...
	isp1763_readl(&dev->regs->chip_id);
	isp1763_readl(&dev->regs->chip_id);
	isp1763_readl(&dev->regs->chip_id);
	mdelay(20);
			       ...
			       resource_size_t res_len)
{
	...
	if (request_irq(ispdev->irq, isp1763_irq, irq_flags, "ISP1763 IRQ",
							(void *) ispdev)) {
		ret = -EIO;
		goto error_free;
	}
					...
					const struct of_device_id *match)
{
	...
	return isp1763_common_init(&dev->dev, devflags, irq, IRQ_SHARED, res, res_len);
...
#define PLX_INT_CSR_REG 0x68
	...
	if (isp1763_common_init(&dev->dev, devflags, dev->irq, IRQ_SHARED,
...
int isp1763_write_done = 0;  /* Is last register access a write? */
EXPORT_SYMBOL_GPL(isp1763_write_done);
...
static int __devinit isp1763_plat_probe(struct platform_device *pdev)
{
	...
	if (isp1763_common_init(&pdev->dev, devflags, irq_res->start, irqflags,
						mem_res, mem_size) < 0)
		goto cleanup;
