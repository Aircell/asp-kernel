...
static int isp1763_resume(struct usb_hcd *hcd)
{
	struct isp1763_hcd *priv = hcd_to_priv(hcd);
	int retval;
	u32 temp32,i;
	...
	for (i = 0; i < 100; i++)
	{
		temp32 = isp1763_readl(hcd->regs + HC_CHIP_ID_REG);
		if(temp32 == ISP_CHIP_ID) 
			break;
		mdelay(1);
	}
	if (i == 100)
...
static int isp1763_suspend(struct usb_hcd *hcd)
{
	struct isp1763_hcd *priv = hcd_to_priv(hcd);
	int retval;
	u32 command;
...
static const struct hc_driver isp1763_hc_driver = {
	...
	.start = isp1763_run,
	.bus_suspend = isp1763_suspend,
	.bus_resume  = isp1763_resume,
	.stop = isp1763_stop,
