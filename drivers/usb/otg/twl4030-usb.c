...
static struct {
	int (*kick_func)(void *kick_data);
	void *kick_data;
} twl4030_kick_data;

static void twl4030_set_kick_interface(int (*kick_func)(void *kick_data), void *kick_data)
{
	twl4030_kick_data.kick_func = kick_func;
	twl4030_kick_data.kick_data = kick_data;
}

static void twl4030_kick_interface(void)
{
	int status=0;

	if (twl4030_kick_data.kick_func)
		status = (*twl4030_kick_data.kick_func)(twl4030_kick_data.kick_data);

	if (status)
		printk("%s: MUSB kick func at %p returned %d\n", __FUNCTION__, twl4030_kick_data.kick_func, status);
}

static void twl4030_vbus_work(struct work_struct *work)
{
       struct twl4030_usb      *twl = container_of(work, struct twl4030_usb, vbus_work);

       if (twl->drive_vbus) {
               (void) twl4030_usb_set_bits(twl, TWL4030_OTG_CTRL,
                               TWL4030_OTG_CTRL_DRVVBUS);
	       twl4030_kick_interface();
       } else {
               (void) twl4030_usb_clear_bits(twl, TWL4030_OTG_CTRL,
                               TWL4030_OTG_CTRL_DRVVBUS);
       }
}
...
static int __devinit twl4030_usb_probe(struct platform_device *pdev)
{
	...
	twl->otg.set_kick_interface = twl4030_set_kick_interface;
