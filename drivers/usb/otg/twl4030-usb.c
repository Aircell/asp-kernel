...
static enum usb_xceiv_events twl4030_usb_linkstat(struct twl4030_usb *twl)
{
	...
	/* Schedule another look in a half a second.
	 * There are some race conditions for the IRQ - if you quickly plug
	 * the device back in, it won't interrupt the replug.
         */
	if(twl->linkstat != linkstat || linkstat == USB_EVENT_ID)
	{
