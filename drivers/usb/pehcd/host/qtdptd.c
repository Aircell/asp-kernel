        ...
        int *status)
{
    ...
    pehci_print ("%s:qtd %p, token %8x bytes %d dma %x\n",
            __FUNCTION__,qtd,  (qtd->hw_token), count, qtd->hw_buf [0]);
        ...
        int  *status)
{
    ...
    if (usb_pipecontrol (urb->pipe)) { 
        /* SETUP pid */
      if(phci_hcd_qtd_fill (urb,qtd, (dma_addr_t) (urb->setup_packet), 
...
//	else
      buf =(dma_addr_t) (urb->transfer_buffer);
//	}
    else
        buf = map_buf = (0); /*set-up stage has no data.*/
        ...
        struct ehci_itd *itd) 
{
    ...
    for(qtd_ptd_index = 1; qtd_ptd_index < max_ptds; qtd_ptd_index++) {                 /* Find the first free slot */
#else
...
phci_hcd_qh_update (phci_hcd *ehci, struct ehci_qh *qh, struct ehci_qtd *qtd)
{
    ...
    wmb (); 
    qh->hw_token &=  (QTD_TOGGLE | QTD_STS_PING);
}
...
phci_hcd_qh_append_tds (
        phci_hcd                *hcd, 
#ifdef LINUX_269
#elif ((defined LINUX_2611) || (defined LINUX_2620))
            ...
            *status = (u32)QUEUE_HEAD_NOT_EMPTY;
            ...
            if (usb_pipebulk (urb->pipe)
                    && unlikely (!usb_gettoggle (urb->dev,
                            (epnum & 0x0f),
//                            !(epnum & 0x10)))) {  /* BUG.BUG.  data toggle mismatch becasue of passing wrong parameter(!(epnum & 0x10)) */
                            !(epnum & 0x80)))) {

                phci_hcd_clear_toggle(urb->dev,
   //                     epnum & 0x0f, !(epnum & 0x10), qh);  /*BUG.BUG.  data toggle mismatch becasue of passing wrong parameter(!(epnum & 0x10)) */
                  epnum & 0x0f, !(epnum & 0x80), qh);
...
phci_hcd_submit_async(
        phci_hcd *hcd,
#if ((defined LINUX_2611) || (defined LINUX_2620))
	struct usb_host_endpoint *ep,
#else
#endif
        struct list_head *qtd_list, 
...
#else
	qh = phci_hcd_qh_append_tds (hcd, urb, qtd_list, &urb->ep->hcpriv,status);
#endif
    if(!qh  || *status < 0)
        goto cleanup;
...
//                            !(epnum & 0x10)))) {  /* BUG.BUG.  data toggle mismatch becasue of passing wrong parameter(!(epnum & 0x10)) */
 ...
 //                       !(epnum & 0x10), 1);  /* BUG.BUG.  data toggle mismatch becasue of passing wrong parameter(!(epnum & 0x10)) */
						!(epnum & 0x80), 1);
        ...
        struct ehci_qh *qh )
{
    ...
    u32     portnum = 0;
    u32     hubnum = 0;
    u32 se = 0,rl = RELOAD_CNT,nk = NAK_CNT;
    u8 datatoggle = 0;
