...
#define HS_QTD_HALT 0x00000040UL 
#define PTD_DATA_LEN_MASK 0x7fff
#define TD_HALTED 0x40
#define EHC_FORCE_HUB_CONFIG 0x14
#define EHC_FORCE_PORT_EN 0x18
#define FS_S_BIT 0x01<<14
#define HS_QHD_H 0x00008000UL
u32 guForceEnSemID;
u32 guForceEnResetPortSemID;
u32 force_enable_cnt=0;
unsigned long GetPortStat[3] = {0x000000a3, 0x00040003};

unsigned long GetPortStat_Setup[5] = { 0x21000041,
                                       0x00000810,
                                       0x00018000,
                                       0x81000000
};
unsigned long GetPortStat_Data[5] = { 0x21000021,
                                      0x00000410,
                                      0x00018000,
                                      0x83000000
};
unsigned long GetPortStat_Handshake[5] = { 0x21000001,
                                           0x00000010,
                                      	   0x00018000,
                                           0x83000000
};

/* Grant, July 3 2008: Added to keep track how many consecutive times the port was force enabled */
int guForceEnPortCnt[3]= {0, 0, 0};
int guForceEnPortHaltCnt[3] = {0,0,0};
#endif
#ifdef SWAP
#define PTD_HED 1
#define PTD_PAY 2
...
#define INVALID_PTR(x) ((((unsigned long)x) <= 0x80000000) || (((unsigned long)x) >= (unsigned long)high_memory))
#endif
...
#ifdef SWAP
	isp1761_mem_write(hcd->dev, td_ptd_map->ptd_header_addr,0,(u32 *)(qha),PHCI_QHA_LENGTH,0,PTD_HED);
#else /* NO_SWAP */
...
#ifdef SWAP
			isp1761_mem_write(hcd->dev,(u32)mem_addr->phy_addr,0,(void*)qtd->hw_buf[0],length,0,PTD_PAY);
#else /* NO_SWAP */
			isp1761_mem_write(hcd->dev,(u32)mem_addr->phy_addr,0,(void*)qtd->hw_buf[0],length,0);
#endif
			break;
		}
	...
	while(schedulemap){
		...
		*/
		...
		if(td_ptd_map->lasttd){
			 ...
			 * */
			qh->hw_current = (u32)(td_ptd_map->qtd);			
			ptd_map_buff->pending_ptd_bitmap &= ~td_ptd_map->ptd_bitmap;
			location++;
			continue;
		}
	...
	loadtd:
		...
		*/
...
#ifdef LOGIC_OMAP3530_SOM_USB1760
		if(INVALID_PTR(urbpriv))
		{ 
			err("Processed invalid pointer on qtd pending schedule.\n");
			location++;
			continue;
		}
#endif       
		...
		if(qtd->length && ( length <= HC_ATL_PL_SIZE))
			...
			{
				int i=0;
				u8 *data_addr= qtd->hw_buf[0];
				printk("Before memory transfer:\n");
				for(i=0;i<length;i++)
					printk("%2x ",*data_addr++);
				printk("\n");
			}
		...
		struct ehci_qh *qh,td_ptd_map_t *td_ptd_map)
{
	...
	urbpriv = (urb_priv_t*)urb->hcpriv;
        ...
        if(INVALID_PTR(urbpriv))
        { 
		err("Processed invalid pointer on qtd schedule.\n");
		return;
        }
#endif       
	urbpriv->timeout = 0;       
		...
		td_ptd_map_t    *td_ptd_map)
{
	...
	if(urb->status == -EINPROGRESS)
		urb->status = 0;
//   printk("compl: urb->status %d\n",urb->status);
	status = usb_hcd_check_unlink_urb(&hcd->usb_hcd, urb, status);
	if (!status)
			...
			if( last_td == TRUE )
			{
				...
				{                                       
					...
					else
					{
						 ...
						 */
						itd = (struct ehci_itd *)(current_itd->hw_next);
...
#ifdef ISP1761_SUDDEN_DISCONNECT_FIX
static int pehci_get_port_status (phci_hcd *hcd , u32 port)
{
	unsigned long uData = 0, skipmap, uTemp;
	unsigned char pbyBuffer[5];	
	unsigned long ptdhdr[9];
	u16 portstatus;
	u32 ptd_no = (u32)(0x01);
	u32 reg=0;
	int i, retryCount=0;
	//To find out the preserve dt bit of controle pipe of Internal hub
	td_ptd_map_t *td_ptd_map;
	td_ptd_map_buff_t *ptd_map_buff;

	ptd_map_buff = &(td_ptd_map_buff[TD_PTD_BUFF_TYPE_ATL]);
	/*read our td_ptd_map*/
	td_ptd_map = &ptd_map_buff->map_list[1];

	while(retryCount<=10){
		skipmap = ptd_no;
		/* Get hub port status for particular port */

		/* Setup stage */
		GetPortStat[1] &= ~0xff;
		GetPortStat[1] |= port;
		isp1761_mem_write(hcd->dev, 0xc00,0,(u32 *)(GetPortStat_Setup),PHCI_QHA_LENGTH,0);
		isp1761_mem_write(hcd->dev, 0x1000,0,(u32 *)(GetPortStat),0x8,0);

		/* Enable OR Mask */	
		uData = isp1761_reg_read32(hcd->dev, hcd->regs.atl_irq_mask_or, uData);
		isp1761_reg_write32(hcd->dev, hcd->regs.atl_irq_mask_or, uData|skipmap);

		/* Now Enable Skipmap */
		uData= isp1761_reg_read32(hcd->dev, hcd->regs.atltdskipmap, uData);
		uData = (uData&~skipmap);
		isp1761_reg_write32(hcd->dev, hcd->regs.atltdskipmap, uData);

		/* Wait for completion of setup stage */
		do {
			for(i=0; i < 10000; i++)
			{
				reg=isp1761_reg_read32(hcd->dev, HC_CHIP_ID_REG,reg);
			}
			isp1761_mem_read(hcd->dev, 0xc00,0,(u32 *)(ptdhdr),PHCI_QHA_LENGTH,0);
#if 0
			printk("setup\n");
			printk(" DW0: 0x%x\n",ptdhdr[0]);
			printk(" DW1: 0x%x\n",ptdhdr[1]);
		...
		isp1761_mem_read(hcd->dev, 0xc00,0,(u32 *)(ptdhdr),PHCI_QHA_LENGTH,0);
		if(ptdhdr[3] & 0xF0000000) 
		{
			retryCount++;
			continue;
		}	
		/* Prevent this PTD from being processed */
		skipmap = isp1761_reg_read32(hcd->dev, hcd->regs.atltdskipmap, skipmap);

		/* Combine the inactive PTDs with the completed PTD */
		skipmap |= ptd_no;
		/* Skip the completed PTD */
		isp1761_reg_write32(hcd->dev, hcd->regs.atltdskipmap, skipmap); 
	
		/* Disable OR Mask */	
		uData= isp1761_reg_read32(hcd->dev, hcd->regs.atl_irq_mask_or, uData); 
		uData &= ~skipmap;
		isp1761_reg_write32(hcd->dev, hcd->regs.atl_irq_mask_or, uData); 
		/* Data stage */
		skipmap = ptd_no;

		isp1761_mem_write(hcd->dev, 0xc00,0,(u32 *)(GetPortStat_Data),PHCI_QHA_LENGTH,0);
		/* Enable OR Mask */	
		uData = isp1761_reg_read32(hcd->dev, hcd->regs.atl_irq_mask_or, uData);
		isp1761_reg_write32(hcd->dev, hcd->regs.atl_irq_mask_or, uData|skipmap);
		...
		uTemp=ptdhdr[3];
		uTemp=uTemp & 0x0F;
		if((ptdhdr[3] & 0xF0000000)||(uTemp<=3))
		{
			retryCount++;
			continue;
		}
		...
		isp1761_mem_read(hcd->dev, 0x1000,0x0,(u32 *)(pbyBuffer),0x4,0);
		portstatus = pbyBuffer[1];
		portstatus = portstatus << 8;
		portstatus = portstatus |pbyBuffer[0];
		/* Handshake stage */
		skipmap = ptd_no;
		isp1761_mem_write(hcd->dev, 0xc00,0,(u32 *)(GetPortStat_Handshake),PHCI_QHA_LENGTH,0);
...
pehci_hcd_intl_worker(phci_hcd *hcd)
{
	...
	while(donetoclear){
...
#ifdef LOGIC_OMAP3530_SOM_USB1760
		//if(!qh || !qtd || !qtd->urb || !(qtd->urb)->hcpriv)
		if(INVALID_PTR(qh) || 
			(INVALID_PTR(qtd)) ||
			(INVALID_PTR(qtd->urb)) ||
			(INVALID_PTR((qtd->urb)->hcpriv)))  
		{
		...
		/*read this ptd from the ram address,address is in the
		  td_ptd_map->ptd_header_addr*/
#ifdef SWAP
		isp1761_mem_read(hcd->dev, td_ptd_map->ptd_header_addr,0,(u32 *)(qhint),PHCI_QHA_LENGTH,0,PTD_HED);
#else /* NO_SWAP */
		isp1761_mem_read(hcd->dev, td_ptd_map->ptd_header_addr,0,(u32 *)(qhint),PHCI_QHA_LENGTH,0);
#endif

		// Ensure that the entire PTD has been executed before continuing
		// (The hardware resets this bit once the PTD is exceuted)
		if (qhint->td_info1 & QHA_VALID)
		{
/*			printk("Bug: We shouldn't have QHA_VALID here - index #%i\n",
			index); */
			ormask |= td_ptd_map->ptd_bitmap;
			isp1761_reg_write32 (hcd->dev, hcd->regs.int_irq_mask_or, ormask);
...
#ifdef ISP1761_SUDDEN_DISCONNECT_FIX 
		{
			...
			hubAddr = (qhint->td_info2 >> 25) & 0x7F;
			/* Determing if the just completed request is LS/FS and the request is not HALTED*/
			if((hubAddr == 2) && ((qhint->td_info2 & FS_S_BIT) != 0) &&
				((qhint->td_info4 & PTD_STATUS_HALTED) == 0)) {

				port = (qhint->td_info2>> 18) & 0x7F;
				/* debug */
				if(port > 3) {
					printk("PORT NUMBER MORE THAN 3\n");
				}
				/* debug */
				
				if(guForceEnPortCnt[port - 1]) {
					guForceEnPortCnt[port - 1] = 0;
					guForceEnPortHaltCnt[port - 1] = 0; 
				}				
			}
		...
		if(qhint->td_info4 & PTD_STATUS_HALTED){
			...
			int retval;
			u32 hubAddr, port, ReqLen=0, TxLen=0;
			u16 portstatus;
	
			hubAddr = (qhint->td_info2 >> 25) & 0x7F;
			if((hubAddr == 2) &&  ((qhint->td_info2 & FS_S_BIT) != 0)) {

				port = (qhint->td_info2 >> 18) & 0x7F;		/* debug */
				if(port > 3) {
					printk("PORT NUMBER MORE THAN 3\n");
				}

				portstatus = pehci_get_port_status(hcd, port);
				if(portstatus) 
					retval = hcd->usb_hcd.driver->force_port_enable(&hcd->usb_hcd, portstatus, port);
				else 
					retval = portstatus;
				if(retval) {
					/* Clear the HALTED bit */
					qhint->td_info4 &= ~PTD_STATUS_HALTED;
		
					/* Check if the request is completed */
					ReqLen = ((qhint->td_info1 & 0x3fff1) >> 3);
					TxLen = (int)qhint->td_info4&PTD_DATA_LEN_MASK;
							
					if(ReqLen == TxLen) { /* Complete the request */
						printk("<Force Enable>Complete the request...\n");
						qhint->td_info4 &= ~(1 << 31);	
					...
					/* Grant, July 3 2008: There are certain conditions where after we re-enable the port
					 * (even only once) that the device just continuously HALT all request, in this case we 
					 * need to reset the port
					 */
					if(guForceEnPortCnt[port-1] != 0) {
						if(guForceEnPortHaltCnt[port - 1] > 5) {
//						printk("<Force Enable Intl>This is BAD...\n");
							guForceEnPortHaltCnt[port - 1] = 0;
							urb->status = -EPROTO;
								
						} else {
							guForceEnPortHaltCnt[port - 1] = guForceEnPortHaltCnt[port - 1] + 1;
						}
					}
	...
	copylength:
...
#ifdef SWAP
				isp1761_mem_read(hcd->dev,(u32)mem_addr->phy_addr,0,(void*)((qtd->hw_buf[0])),length,0,PTD_PAY);
#else /* NO_SWAP */
	                isp1761_mem_read(hcd->dev,(u32)mem_addr->phy_addr,0,(void*)qtd->hw_buf[0],length,0);
#endif
		case OUT_PID:
			urb->actual_length += length;
			qh->hw_current = qtd->hw_next;
		...
		if(qtd->state & QTD_STATE_LAST){
			...
			if(!list_empty(&qh->qtd_list)){
				...
				qtd = list_entry(head->next,struct ehci_qtd, qtd_list);
				td_ptd_map->qtd = qtd;
				qh->hw_current = (u32) (qtd);
				qh->qh_state = QH_STATE_LINKED;

			}else{

				td_ptd_map->qtd = (struct ehci_qtd *) (0);
				qh->hw_current = (0);
				qh->qh_state = QH_STATE_TAKE_NEXT;
				donemap &= ~td_ptd_map->ptd_bitmap;
				ptd_map_buff->pending_ptd_bitmap &= ~td_ptd_map->ptd_bitmap;
	...
	schedule:    
		{
			...
			{
				printk(KERN_INFO "Bad qtd pointer.  :/\n");
				continue;
			}
	...
	while(donetoclear){
		...
		*/
		...
		/*read this ptd from the ram address,address is in the
		  td_ptd_map->ptd_header_addr*/
#ifdef SWAP
		isp1761_mem_read(hcd->dev, td_ptd_map->ptd_header_addr,0,(u32 *)(qha),PHCI_QHA_LENGTH,0,PTD_HED);
#else /* NO_SWAP */
		...
		*/
...
#ifdef ISP1761_SUDDEN_DISCONNECT_FIX 
		{
			...
			hubAddr = (qha->td_info2 >> 25) & 0x7F;
			/* Determing if the just completed request is LS/FS and the request is not HALTED*/
			if((hubAddr == 2) && ((qha->td_info2 & FS_S_BIT) != 0) &&
				((qha->td_info4 & PTD_STATUS_HALTED) == 0)) {

				port = (qha->td_info2>> 18) & 0x7F;
				/* debug */
				if(port > 3) {
					printk("PORT NUMBER MORE THAN 3\n");
				}
		...
		if(qha->td_info4 & PTD_STATUS_HALTED){
			...
			if((hubAddr == 2) &&  ((qha->td_info2 & FS_S_BIT) != 0)) {
				...
				port = (qha->td_info2 >> 18) & 0x7F;		/* debug */
				if(port > 3) {
					printk("PORT NUMBER MORE THAN 3\n");
				}

				portstatus = pehci_get_port_status(hcd, port);
//			printk("ATL Layer :portstatus %x \n",portstatus);
				if(portstatus) 
					retval = hcd->usb_hcd.driver->force_port_enable(&hcd->usb_hcd, portstatus, port);
				else 
					retval = portstatus;
				...
				if(retval) {
					...
					/* Check if the request is completed */
					ReqLen = ((qha->td_info1 & 0x3fff1) >> 3);
					TxLen = (int)qha->td_info4&PTD_DATA_LEN_MASK;
					...
					if(guForceEnPortCnt[port-1] != 0) {
						if(guForceEnPortHaltCnt[port - 1] > 5) {
//								printk("<Force Enable Atll>This is BAD...\n");
							guForceEnPortHaltCnt[port - 1] = 0;
							urb->status = -EPROTO;
	...
	schedule:    
		{
			/*current td comes from qh->hw_current*/        
			ptd_map_buff->pending_ptd_bitmap &= ~td_ptd_map->ptd_bitmap;
			td_ptd_map->qtd = (struct ehci_qtd *)((qh->hw_current));/* BIG ENDIA NOSWAP*/
			qtd = td_ptd_map->qtd;
			ormask |= td_ptd_map->ptd_bitmap;
			ptd_map_buff->active_ptds++;
			pehci_hcd_qtd_schedule(hcd,qtd,qh,td_ptd_map);
...
pehci_hcd_start_controller(phci_hcd *hcd)
{
	...
	hwmode = isp1761_reg_read32(hcd->dev,hcd->regs.hwmodecontrol,0); 
	hwmode &= ~0x100;
	isp1761_reg_write32(hcd->dev,hcd->regs.hwmodecontrol,hwmode);
	if((retval = pehci_hcd_handshake(hcd,hcd->regs.hwmodecontrol, 0x100, 0,1000)) != 0){
		err("Host is not into 16-bit mode in 1000 usecs\n");
		return retval;
	}
...
pehci_hcd_enable_interrupts(phci_hcd *hcd)
{
	...
	/*Configure port 1 as Host Port */
#ifdef PORT1_HOST
	isp1761_reg_write32(hcd->dev,OTG_CONTROL_REG,PORT1_HOST_VALUE);	
#endif
	...
	temp &=0;
	temp = isp1761_reg_read32(hcd->dev, hcd->regs.hwmodecontrol, temp);
	temp |= 0x1;/*enable the global interrupt*/  
	isp1761_reg_write32(hcd->dev, hcd->regs.hwmodecontrol, temp);
...
pehci_hcd_irq(struct usb_hcd *usb_hcd)
{
	...
	if (usb_hcd->state & USB_STATE_SUSPENDED) {
		/* Ack the interrupt */
		intr = isp1761_reg_read32(dev, HC_INTERRUPT_REG, intr);
		isp1761_reg_write32(dev, HC_INTENABLE_REG, 0);
		isp1761_reg_write32(dev, HC_INTERRUPT_REG, intr);
		usb_hcd->state = HC_STATE_RESUMING;
		return IRQ_HANDLED;
	}

	if(!(usb_hcd->state & USB_STATE_READY)){
		pr_info("interrupt handler state not ready yet\n");
		return IRQ_NONE;
	}
	...
	
{
	...
	if(retval){
		err("phci_1761_reset: error failing with status %x\n",retval);
		return retval;
	}
...
#if 0  //Register dump
	u32         temp = 0;
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.command, temp);
	printk("HC_USBCMD_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.usbinterrupt, temp);
	printk("HC_INTERRUPT_REG_EHCI 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.usbstatus, temp);
	printk("HC_USBSTS_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.hcsparams, temp);
	printk("HC_SPARAMS_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.frameindex, temp);
	printk("HC_FRINDEX_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.hwmodecontrol, temp);
	printk("HC_HW_MODE_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.interrupt, temp);
	printk("HC_INTERRUPT_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.interruptenable, temp);
	printk("HC_INTENABLE_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.interruptthreshold, temp);
	printk("HC_INT_THRESHOLD_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.iso_irq_mask_or, temp);
	printk("HC_ISO_IRQ_MASK_OR_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.int_irq_mask_or, temp);
	printk("HC_INT_IRQ_MASK_OR_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.atl_irq_mask_or, temp);
	printk("HC_ATL_IRQ_MASK_OR_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.iso_irq_mask_and, temp);
	printk("HC_ISO_IRQ_MASK_AND_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.int_irq_mask_and, temp);
	printk("HC_INT_IRQ_MASK_AND_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.atl_irq_mask_and, temp);
	printk("HC_ATL_IRQ_MASK_AND_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.buffer_status, temp);
	printk("HC_BUFFER_STATUS_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.reset, temp);
	printk("HC_RESET_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.configflag, temp);
	printk("HC_CONFIGFLAG_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.ports[0], temp);
	printk("HC_PORTSC1_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.isotddonemap, temp);
	printk("HC_ISO_PTD_DONEMAP_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.inttddonemap, temp);
	printk("HC_INT_PTD_DONEMAP_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.atltddonemap, temp);
	printk("HC_ATL_PTD_DONEMAP_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.isotdskipmap, temp);
	printk("HC_ISO_PTD_SKIPMAP_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.inttdskipmap, temp);
	printk("HC_INT_PTD_SKIPMAP_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.atltdskipmap, temp);
	printk("HC_ATL_PTD_SKIPMAP_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.inttdlastmap, temp);
	printk("HC_INTL_PTD_LASTPTD_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.atltdlastmap, temp);
	printk("HC_ATL_PTD_LASTPTD_REG 0x%x\n",temp);
	temp = isp1761_reg_read32(pehci_hcd->dev, pehci_hcd->regs.scratch, temp);
	printk("HC_SCRATCH_REG 0x%x\n",temp);
#endif
	pehci_entry("-- %s: Exit\n",__FUNCTION__);
	return 0;
}
...
pehci_hcd_urb_enqueue(struct usb_hcd *usb_hcd,struct urb *urb,gfp_t mem_flags)
{
	...
	spin_lock_irqsave(&pehci_hcd->lock, irqsave);
	...
	ret = usb_hcd_link_urb_to_ep(usb_hcd, urb);
	if(ret)
	{
		printk("unable to link\n");
...
done_not_linked:
	spin_unlock_irqrestore(&pehci_hcd->lock, irqsave);
...
/*unlink urb*/
static int
pehci_hcd_urb_dequeue(struct usb_hcd  *usb_hcd,struct urb *urb, int _status)
{
	...
	status = usb_hcd_check_unlink_urb(usb_hcd, urb, status);
	if (status || !urb->hcpriv)
		goto done;
	...
	case PIPE_INTERRUPT:
		...
		if(!list_empty(qtd_list))
		{   
			err("Never Error: List must not be empty\n"); /*only one td in my endpoint*/
		}
		break;
	case PIPE_ISOCHRONOUS:
		...
		u16 wIndex,char *buf,u16 wLength)
{
	...
	pehci_rhinfo("%s: wIndex %d typeReq %04x wValue %04x wLength %d\n", __FUNCTION__, wIndex, typeReq, wValue, wLength);
	pehci_info("%s : Enterd\n",__FUNCTION__);
	pehci_info("%s: number of ports %d\n",__FUNCTION__, ports);
	...
	case ClearPortFeature:
		...
		case USB_PORT_FEAT_C_ENABLE:
			...
			isp1761_reg_write32(hcd->dev,hcd->regs.ports[wIndex],(temp & ~PORT_RWC_BITS) | PORT_PEC);
			break;
		...
		case USB_PORT_FEAT_POWER:
			if(ports & 0x10){/*port has has power control switches*/
				isp1761_reg_write32(hcd->dev, hcd->regs.ports[wIndex], temp & ~(PORT_RWC_BITS | PORT_POWER));
			}
			break;
		case USB_PORT_FEAT_C_CONNECTION:
			pehci_print("connect change, status is 0x%08x\n",temp);
			isp1761_reg_write32(hcd->dev, hcd->regs.ports[wIndex], (temp & ~PORT_RWC_BITS) | PORT_CSC);
			break;
		case USB_PORT_FEAT_C_OVER_CURRENT:
			isp1761_reg_write32(hcd->dev, hcd->regs.ports[wIndex], (temp & ~PORT_RWC_BITS) | PORT_OCC);
			break;
		default:
			goto error;
	...
	case SetPortFeature:
		...
		switch (wValue) {
		case USB_PORT_FEAT_ENABLE:
			pehci_info(" SetPortFeature: USB_PORT_FEAT_ENABLE\n");
			//enable the port
			isp1761_reg_write32(hcd->dev, hcd->regs.ports[wIndex], temp | PORT_PE);
			break;
		case USB_PORT_FEAT_SUSPEND:
#if 0
			/* TODO: Enabling this starts a infinite series of connects and
			 * disconnects on the internal hub.  This needs to be investigated
			 * further for power savings.
			 *		  - Jeff Cooper, 11/18/09
			 */
			pehci_info(" SetPortFeature: USB_PORT_FEAT_SUSPEND\n");
			...
			break;
		case USB_PORT_FEAT_POWER:
			pehci_info(" SetPortFeature: USB_PORT_FEAT_POWER\n");
			if(ports & 0x10){
				isp1761_reg_write32(hcd->dev, hcd->regs.ports[wIndex], temp | PORT_POWER);
			}
			break;
		case USB_PORT_FEAT_RESET:
			pehci_info(" SetPortFeature: USB_PORT_FEAT_RESET\n");
			if (temp & PORT_RESUME)
				goto error;
			...
			if ((hcd == NULL) || (hcd->dev == NULL) || (hcd->dev->baseaddress == NULL))
				goto error;
...
#ifdef ISP1761_CONFIG_PM
static int pehci_bus_suspend(struct usb_hcd *hcd)
{
	pehci_entry("++ %s: Entered\n",__FUNCTION__);
	pehci_entry("-- %s: Exit\n",__FUNCTION__);
	return 0;
}
static int pehci_bus_resume(struct usb_hcd *hcd)
{
	pehci_entry("++ %s: Entered\n",__FUNCTION__);
	pehci_entry("-- %s: Exit\n",__FUNCTION__);
	return 0;
...
#else
//pehci_bus_suspend = NULL;
//pehci_bus_resume = NULL;
#endif
...
#ifdef ISP1761_SUDDEN_DISCONNECT_FIX
/* Added by Grant on Aug 29, 2007: Used to convert PTD bitmap into its decimal index equivalent.
 * Example: bitmap: 0x10  decimal index (used in accessing array): 4
 */
unsigned long ConvertBitmapToDec(unsigned long uBitmap) {
	unsigned long temp=0, mask=0x1;
...
/* Grant, Oct 28: should never come here, but just in case */
	pehci_entry("-- %s: Exit\n",__FUNCTION__);
	return temp;
}

static int
pehci_force_port_enable (struct usb_hcd *usb_hcd,u16 portstatus, int port)
{
	u16 temp, temp1;
	u32	xAtlPtds, xIntlPtds, port_enable, reg = 0;
	u32	activPtds = 0, mask, ptdIndex, s_bit, ptdPort, hubAddr;
	u32	ptdhdr[8], speed=0;
	int i;
	td_ptd_map_t *td_ptd_map;
	td_ptd_map_buff_t *ptd_map_buff;
	...
	pehci_entry("++ %s: Entered\n",__FUNCTION__);
	/* Get the speed of the device on the port */
	temp =portstatus;
	temp = portstatus & 0x200; 
	temp1 = portstatus & 0x400;
	
	//printk("portstatus %x portno %d\n",portstatus,port);
	...
	/* Check if the port enable change happen with the internal hub on a port where
	 * a FS/LS device is connected */
	if(((speed && portstatus & 0x1) != 0) && ((portstatus& 0x2) == 0)) {
		/* Check if there is a device on the port when the enable bit was cleared 
		 * Current Connect Status = 1
		 * Port Enabled/Disabled = 0
		 */	
		if(guForceEnPortCnt[port - 1] < 10) {
			
			/* printk( HUB: Port disabled with device conn...\n");  */
			force_enable_cnt ++;
	
			/* Increment the number of times this port has been force enabled 
			 */
			guForceEnPortCnt[port - 1] = guForceEnPortCnt[port - 1] + 1;

			/* Disable all PTDs directed to the problematic port */

			/* Get all ATL PTDs directed to the problematic port */
			mask = 0x1;
			xAtlPtds = 0;
			activPtds= isp1761_reg_read32(hcd->dev, hcd->regs.atltdskipmap, activPtds);
			activPtds = ~activPtds;
			ptd_map_buff = &(td_ptd_map_buff[TD_PTD_BUFF_TYPE_ATL]);
			while(activPtds) {
				if(activPtds & mask) {
					ptdIndex = ConvertBitmapToDec(mask);	
					/*read our td_ptd_map*/
					td_ptd_map = &ptd_map_buff->map_list[ptdIndex];

					/*read this ptd from the ram address,address is in the
					  td_ptd_map->ptd_header_addr*/
					isp1761_mem_read(hcd->dev, td_ptd_map->ptd_header_addr,0,(u32 *)ptdhdr,PHCI_QHA_LENGTH,0);
  
					/* Check only the active PTDs */
					if(ptdhdr[3] & 0x80000000) { 
						s_bit = ptdhdr[1] & FS_S_BIT;
						ptdPort = (ptdhdr[1] >> 18) & 0x7f;
						hubAddr = (ptdhdr[1] >> 25) & 0x7F;

						if((hubAddr == 2) && s_bit && ( ptdPort == port)) {
							xAtlPtds |= mask;
						}
					}
			...
			/* Disable all ATL PTDs for problematic port */

			/* Disable in skipmap */
			activPtds= isp1761_reg_read32(hcd->dev, hcd->regs.atltdskipmap, activPtds);
			activPtds |= xAtlPtds;	
			isp1761_reg_write32(hcd->dev, hcd->regs.atltdskipmap, activPtds);

			/* Disable OR Mask */	
			activPtds= isp1761_reg_read32(hcd->dev, hcd->regs.atl_irq_mask_or, activPtds);
			activPtds &= ~xAtlPtds;
			isp1761_reg_write32(hcd->dev, hcd->regs.atl_irq_mask_or, activPtds);

			/* Get all INTL PTDs directed to the problematic port */
			mask = 0x1;
			xIntlPtds = 0;
			activPtds= isp1761_reg_read32(hcd->dev, hcd->regs.inttdskipmap, activPtds);
			activPtds = ~activPtds;
			...
			/* Disable all INTL PTDs for problematic port */
			/* Disable in skipmap */
			activPtds= isp1761_reg_read32(hcd->dev, hcd->regs.inttdskipmap, activPtds);
			activPtds |= xIntlPtds;	
			isp1761_reg_write32(hcd->dev, hcd->regs.inttdskipmap, activPtds);

			/* Disable OR Mask */	
			activPtds= isp1761_reg_read32(hcd->dev, hcd->regs.int_irq_mask_or, activPtds);
			activPtds &= ~xIntlPtds;
			isp1761_reg_write32(hcd->dev, hcd->regs.int_irq_mask_or, activPtds);

			/* Set the internal hub to force configured state */
			...
			/* Have some delay before we re-enable the port */
			for(i=0; i < 100000; i++);

			/* Set "force configure" bit in Force hub configuration register */
			isp1761_reg_write32(hcd->dev, EHC_FORCE_HUB_CONFIG, 0x1);

			/* Get current value in Force port enable register */
			reg = isp1761_reg_read32(hcd->dev, EHC_FORCE_PORT_EN, reg);

			switch(port)
			{
			case 1:
				/* Clear the current value set for port 1 */
				reg &= ~(0x3 << 0);
				/* Set the desired value base on the device speed */
				reg |= (port_enable << 0);
				break;
			case 2:
				/* Clear the current value set for port 2 */
				reg &= ~(0x3 << 2);
				/* Set the desired value base on the device speed */
				reg |= (port_enable << 2);
				break;
			case 3:
				/* Clear the current value set for port 3 */
				reg &= ~(0x3 << 4);
				/* Set the desired value base on the device speed */
				reg |= (port_enable << 4);
				break;
			...
			/* Update the force enable register */
			isp1761_reg_write32(hcd->dev, EHC_FORCE_PORT_EN, reg);	
			reg= isp1761_reg_read32(hcd->dev, EHC_FORCE_PORT_EN, reg);
			switch(port)
			{
			case 1:
			...
			isp1761_reg_write32(hcd->dev, EHC_FORCE_PORT_EN, reg);	
			reg= isp1761_reg_read32(hcd->dev, EHC_FORCE_PORT_EN, reg);
			/* Clear the force configure bit */
			isp1761_reg_write32(hcd->dev, EHC_FORCE_HUB_CONFIG, 0x00);	

			/* Re-enable paused PTDs */
			if(xAtlPtds || xIntlPtds) {
				for(i=0; i < 200000; i++);
			
				/* Enable all paused ATL PTDs */
				/* Enable OR Mask */	
				activPtds= isp1761_reg_read32(hcd->dev, hcd->regs.atl_irq_mask_or, activPtds);
				activPtds |= xAtlPtds;
				isp1761_reg_write32(hcd->dev, hcd->regs.atl_irq_mask_or, activPtds);
...
/* We only reset the port if a device is still connected to it */
	pehci_entry("-- %s: Exit\n",__FUNCTION__);
	return 0;
}
#endif
...
static const struct hc_driver pehci_driver = {
	...
	.product_desc =     "ST-ERICSSOON ISP1761",
...
#endif
#ifdef ISP1761_SUDDEN_DISCONNECT_FIX
	.force_port_enable = pehci_force_port_enable,
#endif
};
...
{
#ifdef NON_PCI
	struct platform_device *dev = tmp_1761_dev->dev;
#else /* PCI */
	...
	pehci_entry("++ %s: Entered\n",__FUNCTION__);
	if (usb_disabled()) {
		pr_info("++ %s usb disabled!\n", __FUNCTION__);
		return -ENODEV;
	}

	usb_hcd = usb_create_hcd(&pehci_driver, &dev->dev, "ISP1761");

	if (usb_hcd == NULL){
		status = -ENOMEM;
		goto clean;
	}
	pehci_entry("++ %s: Created hcd\n",__FUNCTION__);
	/*this is our host*/
	pehci_hcd = usb_hcd_to_pehci_hcd(usb_hcd);
	...
	usb_hcd->self.controller->dma_mask = 0; /*mask the dma, we use PIO */
	usb_hcd->self.uses_dma = 0;
#endif

	status = usb_add_hcd(usb_hcd, tmp_1761_dev->irq, IRQF_TRIGGER_LOW | IRQF_DISABLED);
...
static isp1761_id ids = {
	.idVendor =   0x04cc, /* isp1761 vendor_id*/
	.idProduct =  0x1A64, /* isp1761 product_id*/
...
static int  __init pehci_module_init(void)
{
	...
	if(!result)
		pehci_info("Host Driver has been Registered");
	else
